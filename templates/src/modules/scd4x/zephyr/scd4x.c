...
static int scd4x_get_serial_number(const struct device *dev)
{
	...
	LOG_DBG("serial: 0x%04x%04x%04x", serial_number0, serial_number1, serial_number2);
							  ...
							  enum sensor_channel chan)
{
	...
	if (cfg->model == SCD4X_MODEL_SCD41 && cfg->measure_mode == SCD4X_MEASURE_MODE_SINGLE_SHOT) {
		...
		if ((chan & SENSOR_CHAN_AMBIENT_TEMP) == chan ||
			(chan & SENSOR_CHAN_HUMIDITY) == chan) {
			...
			// If in single shot mode - this should be repeated
		}
	}
	/*
