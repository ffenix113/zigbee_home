...
```sh
go run ./cmd/zigbee_home --config ./zigbee_test.yml firmware --workdir ./firmware build
```

Only source code can be generated by adding `--only-generate` flag after `build`:
```sh
go run ./cmd/zigbee_home --config ./zigbee_test.yml firmware --workdir ./firmware build --only-generate
```
